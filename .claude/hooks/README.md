...
### 8.5. `block_managed_file_edit.py`

**目的**: keito4/config から自動同期される管理ファイル（`# Managed by keito4/config` マーカー付き）の下流リポジトリでの直接編集をブロック

**トリガー**: `PreToolUse(Edit / Write)`

**動作**:

- 編集対象ファイルの先頭5行にマーカーがあれば exit code 2 でブロックし、config 側テンプレートの修正か `.github/sync-downstream.json` の `exclude` 追加へ誘導
- config リポジトリ自身（`.github/sync-downstream.json` が存在）ではテンプレート編集が正当なためスキップ

参考: ADR 0017 / ADR 0018
