...
- 現在のリポジトリが `keito4/config` 自身の場合はスキップ
- `.devcontainer/devcontainer.json` に `config-base` イメージ参照がない場合はスキップ（config 管理外のリポジトリ）
...
for file in "${TEMPLATE_FILES[@]}"; do
  ...
  if [ ! -f "$DST" ]; then
    ...
    # 差分あり → diff を表示して確認
    echo "📄 $file に差分があります:"
    diff --color=auto -u "$DST" "$SRC" | head -50
    # full mode: ユーザーに確認してから上書き
    # check-only mode: 差分を報告のみ
    if [ "$MODE" = "full" ]; then
      echo "上書きしますか？ (y/n)"
      # Claude による対話的確認
      cp "$SRC" "$DST"
