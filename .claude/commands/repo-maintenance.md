...
| CI/CD Workflows     | `.github/workflows/*.yml/.yaml`  | ワークフロー名と目的                               |
...
4. CI/CD Workflows（`.github/workflows/*.yml` / `.yaml` の `name:` を取得）
...
| #   | チェック                               | 確認方法                                                                        | 推奨                           |
| --- | -------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------ |
| 1   | Node.js バージョン統一                 | `.github/workflows/*.yml` / `.yaml` の `node-version` と `.node-version` を比較 | `.node-version` の値と一致     |
...
WORKFLOW_VERS=$(grep -h 'node-version' "${WORKFLOW_FILES[@]}" 2>/dev/null \
...
# Actions バージョン統一
grep -h 'uses:' "${WORKFLOW_FILES[@]}" 2>/dev/null \
  | sed 's/.*uses: *//' | sort | uniq -c | sort -rn \
  | awk '{print $2}' | sed 's/@.*//' | sort -u \
  | while read action; do
    versions=$(grep -h "uses: *${action}@" "${WORKFLOW_FILES[@]}" 2>/dev/null \
      | sed "s/.*@//" | sort -u)
...
# Runner バージョン
grep -h 'runs-on:' "${WORKFLOW_FILES[@]}" 2>/dev/null \
...
2. 管理対象ワークフローのテンプレートと実体が同期しているか
3. 参照専用テンプレートが本リポジトリ未使用であることを明記しているか
4. `templates/github/` のテンプレート（CODEOWNERS, CONTRIBUTING.md 等）が最新か

**確認ロジック:**

```bash
# 管理対象ワークフローの同期チェック（コメント差分は許容）
npm run workflow:sync:check

# 参照専用テンプレートの明示チェック
grep -L 'Reference-only template; this repository does not install it under .github/workflows/' \
  templates/workflows/e2e-playwright.yml \
  templates/workflows/stale.yml \
  templates/workflows/terraform-drift.yml \
  templates/workflows/claude-health-check.yml && \
  echo "WARN: reference-only workflow template is missing the required header"
...
for script_name in lint typecheck type-check format:check test; do
  ...
  if ! grep -rqE "run (-w [^ ]+ )?$script_name\b|npx $script_name\b" .github/workflows/*.yml .github/workflows/*.yaml 2>/dev/null; then
...
**確認対象:** `.github/workflows/*.yml` / `.yaml` 全ファイル
