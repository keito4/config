...
走らせる。2026-09-04 の freee MCP 復旧時に実測したところ、初回相当の起動に 20.0 秒
...
並行起動でパッケージ展開が衝突する。2026-09-05 に `slack` と `sentry-elu` が同時に
...
```
npm i -g @sentry/mcp-server slack-mcp-server
```
