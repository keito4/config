...
以下のプラグインが事前インストールされています（2026-09-06 の棚卸しで 21 件から 3 件に削減）：
...
事前インストールの対象は `.claude/plugins/plugins.txt` が正本。追加したい場合は同ファイルに
1 行足す（詳細は [.claude/plugins/README.md](../.claude/plugins/README.md)）。
