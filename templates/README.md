...
| `scheduled-maintenance.yml` | config メンテナンス一式を持つリポジトリ | `.takt/`、`script/repo-maintenance.sh`、`script/run-takt-repo-maintenance.sh`、`script/check-trivyignore-review.sh`、`script/validate-takt-auth.sh`、`script/trust-claude-workspace.sh`、`.github/actions/setup-node-ci`、`takt` 依存関係が揃っている場合 |
