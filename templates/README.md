...
| `pr-size-gate.yml`          | PR サイズ上限（400行/25ファイル）強制   | 常に推奨（setup-ci が自動配置。例外は size/override ラベル + Re-run）                                                                                                                                                                                                                        |
| `scheduled-maintenance.yml` | config メンテナンス一式を持つリポジトリ | `.takt/`、`script/repo-maintenance.sh`、`script/run-takt-repo-maintenance.sh`、`script/check-trivyignore-review.sh`、`script/validate-takt-auth.sh`、`script/validate-github-token.sh`、`script/trust-claude-workspace.sh`、`.github/actions/setup-node-ci`、`takt` 依存関係が揃っている場合 |
...
`scheduled-maintenance.yml` は workflow 単体で導入するテンプレートではありません。config 系のメンテナンス実装一式を持つリポジトリ向けの管理コピーです。
